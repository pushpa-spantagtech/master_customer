    ...
    return GetBuilder<RideController>(builder: (rideController) {
      return GetBuilder<LocationController>(builder: (locationController) {
        return Column(children: [
          ...
          RouteWidget(
            totalDistance: rideController.tripDetails == null
                ? rideController.estimatedDistance
                : rideController.tripDetails!.estimatedDistance!.toString(),
            fromAddress: rideController.tripDetails == null
            ...
            toAddress: rideController.tripDetails == null
                ? locationController.toAddress?.address ?? ''
                : rideController.tripDetails!.destinationAddress ?? '',
            entrance: rideController.tripDetails == null
                ? locationController.entranceController.text
                : rideController.tripDetails!.entrance ?? '',
          ),
          const Divider(color: Color.fromRGBO(20, 20, 20, 0.2)),
          RiseFareWidget(
              expandableKey: widget.expandableKey, fromPage: RiseFare.ride),
          const SizedBox(height: Dimensions.paddingSizeDefault),
        ]);
      });
    });

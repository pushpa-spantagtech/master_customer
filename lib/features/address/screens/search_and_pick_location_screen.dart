                              ...
                              onTap: () {
                                if (selectedCount == 0) {
                                  ...
                                  Get.find<LocationController>()
                                    ...
                                    type: Get.find<LocationController>().locationType,
                                    fromSearch: true,
                                  )
                                      .then((value) {
                                    final selectedAddress = value?.address ?? '';
                                    if (locationController.locationType == LocationType.from) {
                                      locationController.pickupLocationController.text = selectedAddress;
                                    } else if (locationController.locationType == LocationType.to) {
                                      locationController.destinationLocationController.text = selectedAddress;
                                    } else if (locationController.locationType == LocationType.extraOne) {
                                      locationController.extraRouteOneController.text = selectedAddress;
                                    } else if (locationController.locationType == LocationType.extraTwo) {
                                      locationController.extraRouteTwoController.text = selectedAddress;
                                    } else {
                                      locationController.locationController.text = selectedAddress.isEmpty ? 'no' : selectedAddress;
                                    }
                                    Get.back();
                                  });
                                }
                        ...
                        RouteHelper.goPageAndHideTextField(
                            ...
                            PickMapScreen(
                              ...
                              onLocationPicked: (Position position, String address) {
                                ...
                                if (locationController.locationType == LocationType.from) {
                                  locationController.pickupLocationController.text = address;
                                } else if (locationController.locationType == LocationType.to) {
